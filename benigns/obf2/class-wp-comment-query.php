...
class WP_Comment_Query {
	...
	protected function get_comment_ids() {
		...
		if ( isset( $this->query_vars['search'] ) && strlen( $this->query_vars['search'] ) ) {
			...
			// Strip leading 'AND'.
			$this->sql_clauses['where']['search'] = __fn_18860( '/^\s*AND\s*/', '', $search_sql );
		}
		...
		if ( ! empty( $this->meta_query_clauses ) ) {
			...
			// Strip leading 'AND'.
			$this->sql_clauses['where']['meta_query'] = __fn_18860( '/^\s*AND\s*/', '', $this->meta_query_clauses['where'] );
		...
		if ( ! empty( $this->query_vars['date_query'] ) && is_array( $this->query_vars['date_query'] ) ) {
			...
			// Strip leading 'AND'.
			$this->sql_clauses['where']['date_query'] = __fn_18860( '/^\s*AND\s*/', '', $this->date_query->get_sql() );
		}
