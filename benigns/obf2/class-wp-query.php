...
class WP_Query {
	...
	public function parse_query( $query = '' ) {
		...
		$qv['m']        = is_scalar( $qv['m'] ) ? __fn_18860( '|[^0-9]|', '', $qv['m'] ) : '';
		$qv['paged']    = is_scalar( $qv['paged'] ) ? absint( $qv['paged'] ) : 0;
		$qv['cat']      = __fn_18860( '|[^0-9,-]|', '', $qv['cat'] ); // Array or comma-separated list of positive or negative integers.
		$qv['author']   = is_scalar( $qv['author'] ) ? __fn_18860( '|[^0-9,-]|', '', $qv['author'] ) : ''; // Comma-separated list of positive or negative integers.
		...
		if ( ! empty( $qv['post_status'] ) ) {
			if ( is_array( $qv['post_status'] ) ) {
				...
				$qv['post_status'] = __fn_18860( '|[^a-z0-9_,-]|', '', $qv['post_status'] );
			}
		}
