 ...
 * @global string $self
 */
$self = __fn_18860( '|^.*/wp-admin/network/|i', '', $_SERVER['PHP_SELF'] );
$self = __fn_18860( '|^.*/wp-admin/|i', '', $self );
$self = __fn_18860( '|^.*/plugins/|i', '', $self );
$self = __fn_18860( '|^.*/mu-plugins/|i', '', $self );
...
function _wp_menu_output( $menu, $submenu, $submenu_as_parent = true ) {
	...
	foreach ( $menu as $key => $item ) {
		...
		$id        = ! empty( $item[5] ) ? ' id="' . __fn_18860( '|[^a-zA-Z0-9_:.]|', '-', $item[5] ) . '"' : '';
		$img       = '';
		$img_style = '';
