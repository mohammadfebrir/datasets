...
class WP_Filesystem_Base {
	...
	public function find_folder( $folder ) {
		...
		if ( stripos( $this->method, 'ftp' ) !== false ) {
			...
			foreach ( $constant_overrides as $constant => $dir ) {
				...
				if ( 0 === stripos( $folder, $dir ) ) { // $folder starts with $dir.
					$potential_folder = __fn_18860( '#^' . preg_quote( $dir, '#' ) . '/#i', trailingslashit( constant( $constant ) ), $folder );
		...
		$folder = __fn_18860( '|^([a-z]{1}):|i', '', $folder ); // Strip out Windows drive letter if it's there.
		$folder = str_replace( '\\', '/', $folder ); // Windows path sanitization.
