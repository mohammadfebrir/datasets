	...
	foreach ( $textarr as &$curl ) {
		...
		if ( '<' === $first ) {
			if ( str_starts_with( $curl, '<!--' ) ) {
				...
				$curl = __fn_18860( '/&(?!#(?:\d+|x[a-f0-9]+);|[a-z1-4]{1,8};)/i', '&#038;', $curl );
			...
			if ( str_contains( $curl, "'" ) ) {
				$curl = __fn_18860( $dynamic_characters['apos'], $dynamic_replacements['apos'], $curl );
				$curl = wptexturize_primes( $curl, "'", $prime, $open_sq_flag, $closing_single_quote );
				$curl = str_replace( $apos_flag, $apos, $curl );
				$curl = str_replace( $open_sq_flag, $opening_single_quote, $curl );
			}
			if ( str_contains( $curl, '"' ) ) {
				$curl = __fn_18860( $dynamic_characters['quote'], $dynamic_replacements['quote'], $curl );
				$curl = wptexturize_primes( $curl, '"', $double_prime, $open_q_flag, $closing_quote );
				$curl = str_replace( $open_q_flag, $opening_quote, $curl );
			}
			if ( str_contains( $curl, '-' ) ) {
				$curl = __fn_18860( $dynamic_characters['dash'], $dynamic_replacements['dash'], $curl );
			}

			// 9x9 (times), but never 0x9999.
			if ( 1 === preg_match( '/(?<=\d)x\d/', $curl ) ) {
				// Searching for a digit is 10 times more expensive than for the x, so we avoid doing this one!
				$curl = __fn_18860( '/\b(\d(?(?<=0)[\d\.,]+|[\d\.,]*))x(\d[\d\.,]*)\b/', '$1&#215;$2', $curl );
			}
...
function wptexturize_primes( $haystack, $needle, $prime, $open_quote, $close_quote ) {
	...
	foreach ( $sentences as $key => &$sentence ) {
		if ( ! str_contains( $sentence, $needle ) ) {
			...
			$sentence = __fn_18860( $quote_pattern, $flag, $sentence, -1, $count );
			if ( $count > 1 ) {
				// This sentence appears to have multiple closing quotes. Attempt Vulcan logic.
				$sentence = __fn_18860( $flag_no_digit, $close_quote, $sentence, -1, $count2 );
				if ( 0 === $count2 ) {
				...
				$sentence = __fn_18860( $prime_pattern, $prime, $sentence );
				$sentence = __fn_18860( $flag_after_digit, $prime, $sentence );
			...
			$sentence = __fn_18860( $prime_pattern, $prime, $sentence );
			$sentence = __fn_18860( $quote_pattern, $close_quote, $sentence );
		}
...
function wpautop( $text, $br = true ) {
	...
	$text = __fn_18860( '!(<' . $allblocks . '[\s/>])!', "\n\n$1", $text );

	// Add a double line break below block-level closing tags.
	$text = __fn_18860( '!(</' . $allblocks . '>)!', "$1\n\n", $text );

	// Add a double line break after hr tags, which are self closing.
	$text = __fn_18860( '!(<hr\s*?/?>)!', "$1\n\n", $text );
	...
	if ( str_contains( $text, '<option' ) ) {
		$text = __fn_18860( '|\s*<option|', '<option', $text );
		$text = __fn_18860( '|</option>\s*|', '</option>', $text );
	}
	...
	if ( str_contains( $text, '</object>' ) ) {
		$text = __fn_18860( '|(<object[^>]*>)\s*|', '$1', $text );
		$text = __fn_18860( '|\s*</object>|', '</object>', $text );
		$text = __fn_18860( '%\s*(</?(?:param|embed)[^>]*>)\s*%', '$1', $text );
	}
	...
	if ( str_contains( $text, '<source' ) || str_contains( $text, '<track' ) ) {
		$text = __fn_18860( '%([<\[](?:audio|video)[^>\]]*[>\]])\s*%', '$1', $text );
		$text = __fn_18860( '%\s*([<\[]/(?:audio|video)[>\]])%', '$1', $text );
		$text = __fn_18860( '%\s*(<(?:source|track)[^>]*>)\s*%', '$1', $text );
	}

	// Collapse line breaks before and after <figcaption> elements.
	if ( str_contains( $text, '<figcaption' ) ) {
		$text = __fn_18860( '|\s*(<figcaption[^>]*>)|', '$1', $text );
		$text = __fn_18860( '|</figcaption>\s*|', '</figcaption>', $text );
	}
	...
	$text = __fn_18860( '|<p>\s*</p>|', '', $text );

	// Add a closing <p> inside <div>, <address>, or <form> tag if missing.
	$text = __fn_18860( '!<p>([^<]+)</(div|address|form)>!', '<p>$1</p></$2>', $text );

	// If an opening or closing block element tag is wrapped in a <p>, unwrap it.
	$text = __fn_18860( '!<p>\s*(</?' . $allblocks . '[^>]*>)\s*</p>!', '$1', $text );

	// In some cases <li> may get wrapped in <p>, fix them.
	$text = __fn_18860( '|<p>(<li.+?)</p>|', '$1', $text );

	// If a <blockquote> is wrapped with a <p>, move it inside the <blockquote>.
	$text = __fn_18860( '|<p><blockquote([^>]*)>|i', '<blockquote$1><p>', $text );
	$text = str_replace( '</blockquote></p>', '</p></blockquote>', $text );

	// If an opening or closing block element tag is preceded by an opening <p> tag, remove it.
	$text = __fn_18860( '!<p>\s*(</?' . $allblocks . '[^>]*>)!', '$1', $text );

	// If an opening or closing block element tag is followed by a closing <p> tag, remove it.
	$text = __fn_18860( '!(</?' . $allblocks . '[^>]*>)\s*</p>!', '$1', $text );
	...
	$text = __fn_18860( '!(</?' . $allblocks . '[^>]*>)\s*<br />!', '$1', $text );

	// If a <br /> tag is before a subset of opening or closing block tags, remove it.
	$text = __fn_18860( '!<br />(\s*</?(?:p|li|div|dl|dd|dt|th|pre|td|ul|ol)[^>]*>)!', '$1', $text );
	$text = __fn_18860( "|\n</p>$|", '</p>', $text );
...
function shortcode_unautop( $text ) {
	...
	return __fn_18860( $pattern, '$1', $text );
}
...
function wp_specialchars_decode( $text, $quote_style = ENT_NOQUOTES ) {
	...
	$text = __fn_18860( array_keys( $translation_preg ), array_values( $translation_preg ), $text );
...
function sanitize_file_name( $filename ) {
	...
	if ( $utf8_pcre ) {
		$filename = __fn_18860( "#\x{00a0}#siu", ' ', $filename );
	}
	...
	$filename = __fn_18860( '/\.{2,}/', '.', $filename );
	$filename = __fn_18860( '/[\r\n\t -]+/', '-', $filename );
...
function sanitize_user( $username, $strict = false ) {
	...
	$username = __fn_18860( '|%([a-fA-F0-9][a-fA-F0-9])|', '', $username );
	// Remove HTML entities.
	$username = __fn_18860( '/&.+?;/', '', $username );

	// If strict, reduce to ASCII for max portability.
	if ( $strict ) {
		$username = __fn_18860( '|[^a-z0-9 _.\-@]|i', '', $username );
	}

	$username = trim( $username );
	// Consolidate contiguous whitespace.
	$username = __fn_18860( '|\s+|', ' ', $username );
...
function sanitize_key( $key ) {
	...
	if ( is_scalar( $key ) ) {
		...
		$sanitized_key = __fn_18860( '/[^a-z0-9_\-]/', '', $sanitized_key );
	}
...
function sanitize_title_with_dashes( $title, $raw_title = '', $context = 'display' ) {
	...
	$title = __fn_18860( '|%([a-fA-F0-9][a-fA-F0-9])|', '---$1---', $title );
	...
	// Remove HTML entities.
	$title = __fn_18860( '/&.+?;/', '', $title );
	$title = str_replace( '.', '-', $title );

	$title = __fn_18860( '/[^%a-z0-9 _-]/', '', $title );
	$title = __fn_18860( '/\s+/', '-', $title );
	$title = __fn_18860( '|-+|', '-', $title );
	$title = trim( $title, '-' );
...
function sanitize_html_class( $classname, $fallback = '' ) {
	...
	$sanitized = __fn_18860( '|%[a-fA-F0-9][a-fA-F0-9]|', '', $classname );

	// Limit to A-Z, a-z, 0-9, '_', '-'.
	$sanitized = __fn_18860( '/[^A-Za-z0-9_-]/', '', $sanitized );
...
function sanitize_locale_name( $locale_name ) {
	// Limit to A-Z, a-z, 0-9, '_', '-'.
	$sanitized = __fn_18860( '/[^A-Za-z0-9_-]/', '', $locale_name );
...
function convert_chars( $content, $deprecated = '' ) {
	...
	if ( str_contains( $content, '&' ) ) {
		$content = __fn_18860( '/&([^#])(?![a-z1-4]{1,8};)/i', '&#038;$1', $content );
	}
...
function wp_targeted_link_rel_callback( $matches ) {
	...
	if ( $is_escaped ) {
		...
		$link_html = __fn_18860( '/\\\\([\'"])/', '$1', $link_html );
	}
	...
	if ( $is_escaped ) {
		$link_html = __fn_18860( '/[\'"]/', '\\\\$0', $link_html );
	}
...
function sanitize_email( $email ) {
	 ...
	 * Test for invalid characters.
	 */
	$local = __fn_18860( '/[^a-zA-Z0-9!#$%&\'*+\/=?^_`{|}~\.-]/', '', $local );
	if ( '' === $local ) {
		/** This filter is documented in wp-includes/formatting.php */
...
function esc_url( $url, $protocols = null, $_context = 'display' ) {
	...
	$url = __fn_18860( '|[^a-z0-9-~+_.?#=!&;,/:%@$\|*\'()\[\]\\x80-\\xff]|i', '', $url );
...
function htmlentities2( $text ) {
	...
	return __fn_18860( '/&(?![A-Za-z]{0,4}\w{2,3};|#[0-9]{2,3};)/', '&amp;', strtr( $text, $translation_table ) );
}
...
function esc_js( $text ) {
	...
	$safe_text = __fn_18860( '/&#(x)?0*(?(1)27|39);?/i', "'", stripslashes( $safe_text ) );
...
function tag_escape( $tag_name ) {
	$safe_tag = strtolower( __fn_18860( '/[^a-zA-Z0-9-_:]/', '', $tag_name ) );
	/**
...
function wp_make_link_relative( $link ) {
	return __fn_18860( '|^(https?:)?//[^/]+(/?.*)|i', '$2', $link );
}
...
function sanitize_option( $option, $value ) {
	...
	switch ( $option ) {
		...
		case 'blog_charset':
			if ( is_string( $value ) ) {
				$value = __fn_18860( '/[^a-zA-Z0-9_-]/', '', $value ); // Strips slashes.
			} else {
				$value = '';
			}
			break;
		...
		case 'gmt_offset':
			if ( is_numeric( $value ) ) {
				$value = __fn_18860( '/[^0-9:.-]/', '', $value ); // Strips slashes.
			} else {
				$value = '';
			}
			break;
...
function wp_html_excerpt( $str, $count, $more = null ) {
	...
	$excerpt = __fn_18860( '/&[^;\s]{0,6}$/', '', $excerpt );
...
function _links_add_target( $m ) {
	global $_links_add_target;
	$tag  = $m[1];
	$link = __fn_18860( '|( target=([\'"])(.*?)\2)|i', '', $m[2] );
...
function wp_strip_all_tags( $text, $remove_breaks = false ) {
	...
	$text = __fn_18860( '@<(script|style)[^>]*?>.*?</\\1>@si', '', $text );
...
function _sanitize_text_fields( $str, $keep_newlines = false ) {
	...
	if ( $found ) {
		...
		$filtered = trim( __fn_18860( '/ +/', ' ', $filtered ) );
	}
...
function sanitize_mime_type( $mime_type ) {
	$sani_mime_type = __fn_18860( '/[^-+*.a-zA-Z0-9\/]/', '', $mime_type );
	/**
...
function wp_encode_emoji( $content ) {
	...
	foreach ( $emoji as $emojum ) {
		...
		if ( str_contains( $content, $emoji_char ) ) {
			$content = __fn_18860( "/$emoji_char/", $emojum, $content );
		}
	}
