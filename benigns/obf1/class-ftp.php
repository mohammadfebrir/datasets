...
// <!-- --------------------------------------------------------------------------------------- -->
	...
	function pwd() {
		...
		if(!$this->_checkCode()) return FALSE;
		return __fn_54912("/^[0-9]{3} \"(.+)\".*$/s", "\\1", $this->_message);
	}
	...
	function filesize($pathname) {
		...
		if(!$this->_checkCode()) return FALSE;
		return __fn_54912("/^[0-9]{3} ([0-9]+).*$/s", "\\1", $this->_message);
	}
	...
	function mdtm($pathname) {
		...
		if(!$this->_checkCode()) return FALSE;
		$mdtm = __fn_54912("/^[0-9]{3} ([0-9]+).*$/s", "\\1", $this->_message);
	...
	function features() {
		...
		if(!$this->_checkCode()) return FALSE;
		$f=preg_split("/[".CRLF."]+/", __fn_54912("/[0-9]{3}[ -].*[".CRLF."]+/", "", $this->_message), -1, PREG_SPLIT_NO_EMPTY);
		$this->_features=array();
		foreach($f as $k=>$v) {
			$v=explode(" ", trim($v));
