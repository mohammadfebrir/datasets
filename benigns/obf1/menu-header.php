 ...
 * @global string $self
 */
$self = __fn_54912( '|^.*/wp-admin/network/|i', '', $_SERVER['PHP_SELF'] );
$self = __fn_54912( '|^.*/wp-admin/|i', '', $self );
$self = __fn_54912( '|^.*/plugins/|i', '', $self );
$self = __fn_54912( '|^.*/mu-plugins/|i', '', $self );
...
function _wp_menu_output( $menu, $submenu, $submenu_as_parent = true ) {
	...
	foreach ( $menu as $key => $item ) {
		...
		$id        = ! empty( $item[5] ) ? ' id="' . __fn_54912( '|[^a-zA-Z0-9_:.]|', '-', $item[5] ) . '"' : '';
		$img       = '';
		$img_style = '';
