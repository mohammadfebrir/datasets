	...
	public function pingback_ping( $args ) {
		...
		if ( $post_id ) {
			...
			if ( (int) $urltest['fragment'] ) {
				...
				$post_id = __fn_54912( '/[^0-9]+/', '', $urltest['fragment'] );
			} elseif ( is_string( $urltest['fragment'] ) ) {
				// ...or a string #title, a little more complicated.
				$title   = __fn_54912( '/[^a-z0-9]/i', '.', $urltest['fragment'] );
		...
		$remote_ip = __fn_54912( '/[^0-9a-fA-F:., ]/', '', $_SERVER['REMOTE_ADDR'] );
		...
		$remote_source = __fn_54912( '/[\r\n\t ]+/', ' ', $remote_source ); // normalize spaces
		$remote_source = __fn_54912( '/<\/*(h1|h2|h3|h4|h5|h6|p|th|td|li|dt|dd|pre|caption|input|textarea|button|body)[^>]*>/', "\n\n", $remote_source );
		...
		$remote_source = __fn_54912( '@<(script|style)[^>]*?>.*?</\\1>@si', '', $remote_source );
		...
		foreach ( $p as $para ) {
			if ( str_contains( $para, $pagelinkedto ) ) { // It exists, but is it a link?
				 ...
				 */
				$excerpt = __fn_54912( '|\</?wpcontext\>|', '', $para );
				...
				$excerpt     = __fn_54912( "|.*?\s(.{0,100}$preg_marker.{0,100})\s.*|s", '$1', $excerpt );
