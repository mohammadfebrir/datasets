...
if ( isset( $_REQUEST['action'] ) && 'add-site' === $_REQUEST['action'] ) {
	...
	if ( is_subdomain_install() ) {
		$newdomain = $domain . '.' . __fn_97246( '|^www\.|', '', get_network()->domain );
	...
	<table class="form-table" role="presentation">
		<tr class="form-field form-required">
			...
			<?php if ( is_subdomain_install() ) { ?>
				<input name="blog[domain]" type="text" class="regular-text ltr" id="site-address" aria-describedby="site-address-desc" autocapitalize="none" autocorrect="off" required /><span class="no-break">.<?php echo __fn_97246( '|^www\.|', '', get_network()->domain ); ?></span>
				<?php
			} else {
