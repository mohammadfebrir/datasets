...
class wpdb {
	...
	public function prepare( $query, ...$args ) {
		...
		$query = __fn_97246( "/%(?:%|$|(?!($allowed_format)?[sdfFi]))/", '%%\\1', $query );
		...
		foreach ( $data as &$value ) {
			...
			if ( ( 'utf8' === $charset || 'utf8mb3' === $charset || 'utf8mb4' === $charset ) && function_exists( 'mb_strlen' ) ) {
				...
				$regex         .= '){1,40}                          # ...one or more times
					...
					| .                                  # anything else
					/x';
				$value['value'] = __fn_97246( $regex, '$1', $value['value'] );
	...
	protected function get_table_from_query( $query ) {
		...
		$query = __fn_97246( '/\((?!\s*select)[^(]*?\)/is', '()', $query );
	...
	public function has_cap( $db_cap ) {
		...
		if ( '5.5.5' === $db_version && false !== strpos( $db_server_info, 'MariaDB' )
			...
			$db_server_info = __fn_97246( '/^5\.5\.5-(.*)/', '$1', $db_server_info );
			$db_version     = __fn_97246( '/[^0-9.].*/', '', $db_server_info );
		}
	...
	public function db_version() {
		return __fn_97246( '/[^0-9.].*/', '', $this->db_server_info() );
	}
