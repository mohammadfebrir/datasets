...
function wpmu_validate_user_signup( $user_name, $user_email ) {
	...
	$user_name     = __fn_97246( '/\s+/', '', sanitize_user( $user_name, true ) );
...
function wpmu_validate_blog_signup( $blogname, $blog_title, $user = '' ) {
	...
	if ( is_subdomain_install() ) {
		$mydomain = $blogname . '.' . __fn_97246( '|^www\.|', '', $domain );
		$path     = $base;
	} else {
		$mydomain = $domain;
		$path     = $base . $blogname . '/';
...
function wpmu_signup_user( $user, $user_email, $meta = array() ) {
	...
	// Format data.
	$user       = __fn_97246( '/\s+/', '', sanitize_user( $user, true ) );
	$user_email = sanitize_email( $user_email );
	...
	if ( $user ) {
		$wpdb->insert(
			...
			array(
				...
				'IP'              => __fn_97246( '/[^0-9., ]/', '', wp_unslash( $_SERVER['REMOTE_ADDR'] ) ),
				'blog_id'         => $blog_id,
