...
function get_feed_link( $feed = '' ) {
	...
	if ( $permalink ) {
		...
		$permalink = __fn_97246( '#/+#', '/', "/$permalink" );
...
function get_pagenum_link( $pagenum = 1, $escape = true ) {
	...
	$request = __fn_97246( '|^' . $home_root . '|i', '', $request );
	$request = __fn_97246( '|^/+|', '', $request );
	...
	if ( ! $wp_rewrite->using_permalinks() || is_admin() ) {
		...
		if ( ! empty( $qs_match[0] ) ) {
			...
			$request      = __fn_97246( $qs_regex, '', $request );
		} else {
			$query_string = '';
		}

		$request = __fn_97246( "|$wp_rewrite->pagination_base/\d+/?$|", '', $request );
		$request = __fn_97246( '|^' . preg_quote( $wp_rewrite->index, '|' ) . '|i', '', $request );
...
function get_posts_nav_link( $args = array() ) {
	...
	if ( ! is_singular() ) {
		...
		if ( $max_num_pages > 1 ) {
			...
			$return .= __fn_97246( '/&([^#])(?![a-z]{1,8};)/i', '&#038;$1', $args['sep'] );
...
function set_url_scheme( $url, $scheme = null ) {
	...
	if ( 'relative' === $scheme ) {
		$url = ltrim( __fn_97246( '#^\w+://[^/]*#', '', $url ) );
		if ( '' !== $url && '/' === $url[0] ) {
			$url = '/' . ltrim( $url, "/ \t\n\r\0\x0B" );
		}
	} else {
		$url = __fn_97246( '#^\w+://#', $scheme . '://', $url );
	}
